...
BOT_USERNAME = None
try:
    me = telebot_bot.get_me()
    if me and getattr(me, "username", None):
        BOT_USERNAME = me.username
except Exception:
    BOT_USERNAME = None

SESSION_NAME = BOT_USERNAME if BOT_USERNAME else "video_downloader_bot"

flask_app = Flask(__name__)
pyro_client = Client(SESSION_NAME, api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
...
async def process_download(client, message, url):
    ...
    try:
        ...
        if result is None:
            await message.reply("Oops! This video is longer than 40 minutes, so I can't download it. Sorry about that! 😅")
        elif result == "ERROR":
            await message.reply("Something went wrong while downloading. Please try again! 😓")
        else:
...
async def start_handler(client, message: Message):
    await message.reply(
        "👋 Hello there!\n"
        "Send me a video link from the platforms below, and I'll download it for you instantly! 🚀\n\n"
        "Supported sites:\n"
        "• YouTube\n"
        "• Facebook\n"
        "• Pinterest\n"
...
def handle_online(message):
    telebot_bot.send_message(message.chat.id, "Yes, I'm online and ready! ✅")
