...
async def get_bot_statistics():
    ...
    for item in most_downloaded_sites:
        most_downloads_text += f"• {item['_id']}: {item['count']} jeer\n"
    if not most_downloads_text:
        most_downloads_text = "Wax soo dajin ah wali lama diiwaan gelin."

    return (
        f"📊 **Tirakoobyada Botka** 📊\n\n"
        f"**USERS:**\n"
        f"• Total Users (Dadka isdiiwaangeliyay): **{total_users:,}**\n"
        f"• Dadka firfircoon Bishii u dambaysay (Last Month): **{active_users_last_month:,}**\n"
        f"• Dadka firfircoon Toddobaadkii u dambaysay (Last Week): **{active_users_last_week:,}**\n"
        f"• Dadka firfircoon Maalintii u dambaysay (Last Day): **{active_users_last_day:,}**\n\n"
        f"**DOWNLOADS:**\n"
        f"• Tirada guud ee la soo dajiyay (Total Downloads): **{total_downloads:,}**\n\n"
        f"**Baraha ugu badan ee wax laga soo dajiyay (Most Downloads):**\n"
