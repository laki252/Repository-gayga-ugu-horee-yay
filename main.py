...
WEBHOOK_URL_BASE = os.environ.get("https://repository-gayga-ugu-horee-yay.onrender.com")
if not WEBHOOK_URL_BASE:
    print("FATAL ERROR: WEBHOOK_URL_BASE environment variable not set!")
    print("Fadlan ku dar URL-kaaga asaaska ah (e.g., https://your-app-name.on-render.com)")
...
@quart_app.route("/", methods=["GET", "POST", "HEAD"])
...
async def main():
    ...
    try:
        ...
        print(f"ERROR: Failed to set webhook at {WEBHOOK_URL}. {e}")
    
    port = int(os.environ.get("PORT", 8080))
    await quart_app.run_task(host="0.0.0.0", port=port, debug=False)
